...
class SigLipAttention(nn.Module):
    ...
    def forward(
        ...
        # Apply proportional attention from ToME
        if size is not None:
        ...
        # Return k as well here from ToME
...
class SigLipEncoderLayer(nn.Module):
    def __init__(self, config: SigLipVisionConfig):
        ...
        self._tome_info=config.tome_info
    # Ignore copy
    def forward(
        self,
        hidden_states: torch.Tensor,
        ...
        hidden_states, attn_weights, k_metric = self.self_attn(
...
class SigLipEncoder(nn.Module):
    ...
    def __init__(self, config: SigLipVisionConfig):
        super().__init__()
        config.tome_config["r"]=parse_r(config.num_hidden_layers, config.merging_r)
        self.config = config
...
class SigLipVisionModel(SigLipPreTrainedModel):
    ...
    def __init__(self, config: SigLipVisionConfig):
        ...
        config.tome_config = tome_config
        rank0_print("vit config after adding ToME", config)
...
class ToMESigLipVisionTower(nn.Module):
    def __init__(self, vision_tower, vision_tower_cfg, delay_load=False):
        ...
        rank0_print("merging r",getattr(vision_tower_cfg, "merging_r"))
        rank0_print("trend r", getattr(vision_tower_cfg, "trend"))
        rank0_print("initial self config",self.config)
