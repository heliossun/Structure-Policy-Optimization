...
class ModelArguments:
    ...
    pretrain_mm_vit: Optional[str] = field(default=None)
...
def preprocess_qwen_sq2(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.",sq_r=0.3) -> Dict:
    # roles = {"human": "<|im_start|>user", "gpt": "<|im_start|>assistant"}
...
def preprocess_qwen_sq(
    ...
    for input_id, conversation, target in zip(input_ids, conversations, targets):
        ...
        for conv_idx in range(3, len(rounds), 2):
            re_rounds.append(myConovsep.join(
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if "vuser" not in parts[0]:
                ...
                if has_image:
                    ...
                    round_len = len(tokenizer(rou).input_ids)+2
                    sep_len = len(tokenizer(sep_vur).input_ids)+1
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False,sq_r = 0.3) -> Dict:
    """
    ...
    if conversation_lib.default_conversation.version == "qwen_sq":
        return preprocess_qwen_sq2(sources, tokenizer, has_image=has_image, sq_r=sq_r)
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        data_dict = preprocess(sources, self.tokenizer, has_image=has_image,sq_r=self.sq_r)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.ToME is not None:
        overwrite_config["ToME"] = model_args.ToME
        overwrite_config["merging_r"]=model_args.merging_r
        overwrite_config["trend"]=model_args.trend
    if overwrite_config:
